...
const services = [
  { name: "BIS — CRS", short: "CRS", desc: "Compulsory Registration Scheme", icon: "🛡️", img: "/bis-crs.png" },
  { name: "BIS — ISI", short: "ISI", desc: "Indian Standards Institute Mark", icon: "✅", img: "/bis-isi.png" },
  { name: "WPC — ETA", short: "WPC", desc: "Wireless Planning & Coordination", icon: "📡", img: "/wpc-eta.png" },
  { name: "Testing", short: "TEST", desc: "Product & Safety Testing", icon: "🔬", img: "/testing.png" },
  { name: "BEE", short: "BEE", desc: "Bureau of Energy Efficiency", icon: "⚡", img: "/bee.png" },
  { name: "ISO", short: "ISO", desc: "International Standards", icon: "🌐", img: "/iso.png" },
  { name: "EPR", short: "EPR", desc: "Extended Producer Responsibility", icon: "♻️", img: "/epr.png" },
  { name: "TEC / MTCTE", short: "TEC", desc: "Telecom Equipment Certification", icon: "📶", img: "/tec.png" },
  { name: "LMPC", short: "LMPC", desc: "Legal Metrology Packaged Commodities", icon: "⚖️", img: "/lmpc.png" },
  { name: "CDSCO", short: "CDSCO", desc: "Drug & Medical Device License", icon: "💊", img: "/cdsco.png" },
];
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:items-stretch">
          ...
          <div className="flex flex-col gap-0">
            ...
            <div
              ...
              style={{ border: `1px solid ${COLORS.border}`, borderTop: "none", minHeight: 380 }}
            >
              <img
                src={svc.img}
                alt={svc.name}
              ...
              {/* Top-right badge — uses active service color */}
              <div
                className="absolute right-3 top-3 rounded px-3 py-1.5 text-center text-xs"
                style={{
              ...
              >
                <p className="text-base font-black">{svc.short}</p>
                <p style={{ fontSize: 10, opacity: 0.75 }}>Certified</p>
              </div>

              {/* Floating service detail card */}
              <div
                className="absolute bottom-1 left-4 mt-16 right-16 rounded-xl p-2 transition-all duration-300"
                style={{
              ...
              >
                <p className="mb-1 text-[13px] font-extrabold" style={{ color: "#ff5e24", fontWeight: 800, fontFamily: 'system-ui, sans-serif' }}>{svc.icon} {svc.name}</p>
                <p className="mb-2 text-[10px]" style={{ color: "#ffffffbe", fontFamily: 'var(--font-body)' }}>{svc.desc}</p>
                <div className="h-px mb-1" style={{ background: COLORS.border }} />
                <ul className="grid grid-cols-2 gap-x-3 gap-y-1 text-[11px]" style={{ color: COLORS.muted }}>
                  {["End-to-end management", "Document preparation", "Govt. liaison & follow-up", "Post-cert support"].map((item) => (
                    <li key={item} className="flex items-center gap-1.5 text-[#fff] font-semibold font-body text-[13px]">
              ...
              {/* Progress dots */}
              <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 flex-row gap-1.5">
                {services.map((_, i) => (
                  <div
                    key={i}
